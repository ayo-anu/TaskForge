...
recovery-test:
	...
	uv run pytest tests/integration/test_recovery_scanner.py tests/integration/test_recovery_transition.py tests/integration/test_stale_worker_recovery.py tests/integration/test_worker_crash_boundaries.py
...
m21-workload:
	@test "$${TASKFORGE_RUN_M21_WORKLOAD:-}" = "1" || (echo "TASKFORGE_RUN_M21_WORKLOAD=1 is required" >&2; exit 2)
